...
영속화하고, 선택적으로 공통 `watchdog`의 Palworld 어댑터를
함께 실행합니다.
...
- `make up`: Palworld와 Watchpal 실행
- `make up-palworld`: 게임 서버만 실행
- `make up-watchpal`: Watchpal만 실행
...
적용합니다. 엔트리포인트는 Watchpal 연동을 위해 아래 세 항목만 시작 시
보정합니다.
...
| 단계 | 기본값 | 동작 |
|---|---:|---|
| 주의 | 11GiB | Discord 또는 로그에 주의 알림 |
| 자동 재기동 | 12GiB, 3회 연속 | 접속자 안내, Save, Shutdown |
| 긴급 재기동 | 13.5GiB | 즉시 Save 및 Shutdown 시도 |
| 컨테이너 한도 | 14GiB | OOM 대비 최종 제한 |

Watchpal은 10초마다 메모리를 확인하고 10분마다 REST Save를 수행합니다.
재기동 후 기본 30분의 쿨다운을 적용합니다. `WATCHPAL_WEBHOOK_URL`이 비어 있으면
Discord 전송 없이 같은 내용을 로그에 기록합니다.

Watchpal 이미지는 형제 저장소에서 빌드합니다.

```text
../watchdog/container
```

다른 경로를 사용할 때는 `.env`의 `WATCHDOG_BUILD_CONTEXT`를 변경합니다.

## ARM64 안정성 설정

기본값은 현재 로컬 기동 검증에 사용한 값입니다. 무작위 충돌이 발생하면
아래처럼 안정성을 우선하도록 조정할 수 있습니다.
